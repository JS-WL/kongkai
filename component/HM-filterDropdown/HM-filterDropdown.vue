					...
					<view class="filter">
						<view class="date_wrap">
							<view class="date_content">
								...
								<button @click="onShowDatePicker('date')" style="height: 60upx; font-size: 32upx; line-height: 60upx; margin:0">选择日期</button>
							</view>
							<mx-date-picker :show="showPicker" :type="type" :value="value" :show-tips="false" @confirm="onSelected" @cancel="onSelected" />
						</view>
						<view class="btn-box">
							<view class="reset" @tap="resetFilterData(page_index)">重置</view>
							<view class="submit" @tap="setFilterData(page_index)">确定</view>
		...
		methods: {
			...
			hideMask() {
				...
				setTimeout(() => {
					...
					this.$emit('handleMask', this.maskVisibility)
				}, 200);
			},
			//show遮罩层
			showMask() {
				...
				this.$nextTick(() => {
					setTimeout(() => {
						...
						this.$emit('handleMask', this.isShowMask)
					}, 0);
				})
			},
			//hide菜单页
